...
import { fetchCustomerJourneyData, calculatePremiumWithContext, setSelectedProduct } from '../../slices/productSlice';
...
export default function ProductsPage() {
  ...
  useEffect(() => {
    if (products.length === 0 || insuranceTypes.length === 0) {
      dispatch(fetchCustomerJourneyData({ userId: user?.id }));
    }
  }, [dispatch, products.length, insuranceTypes.length, user?.id]);
  ...
  const handleGetQuote = (product: PolicyProduct) => {
    ...
    // Pre-load customer journey data for this product
    dispatch(fetchCustomerJourneyData({ 
      userId: user?.id, 
  ...
  const handleBuyPolicy = async (product: PolicyProduct) => {
    ...
    try {
      // Fetch all customer journey data in single API call
      const result = await dispatch(fetchCustomerJourneyData({ 
        userId: user.id, 
        productId: product.productId 
      })).unwrap();
      
      const userData = result.userData;
      const userProposals = result.userProposals;
      ...
      // Check for existing proposals for this product
      const existingProposal = userProposals?.find((p: any) => p.productId === product.productId);
      ...
      if (existingProposal) {
        ...
        switch (Number(existingProposal.status)) {
          ...
          case 3: // Approved - check documents
            const proposalDocs = result.userDocuments?.filter((d: any) => d.proposalId === existingProposal.proposalId) || [];
            ...
            } else if (proposalDocs.some((d: any) => Number(d.status) === 18 || Number(d.status) === 2)) {
              toast.info('Documents uploaded, awaiting verification');
              navigate('/kyc');
            } else if (proposalDocs.every((d: any) => Number(d.status) === 19)) {
          ...
          case 4: // Rejected
            ...
            navigate('/proposals', { state: { selectedProduct: product } });
            break;
  ...
  return (
    <Container maxWidth="xl" sx={{ px: { xs: 2, sm: 3 } }}>
                ...
                >
                  ...
                  >
                    <Box
                      sx={{
                        width: '100%',
                        maxWidth: { xs: 'none', sm: '500px', md: '600px' },
      ...
      {/* Login Prompt Dialog */}
      <Dialog open={loginPromptOpen} onClose={() => setLoginPromptOpen(false)} disablePortal>
